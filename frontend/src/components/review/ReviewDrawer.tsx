...
export const ReviewDrawer: React.FC = () => {
  ...
  // Heuristic to label the section header: any CREATE TABLE means a brand-new
  // physical table; otherwise the approval will run ALTERs against an existing
  // one. Falls back to a neutral label when there are no pending changes.
  const ddlKind: "create" | "alter" | "none" = ddlStatements.length === 0
    ? "none"
    ...
    >
      {tableDef ? (
        <div className="space-y-6">
          ...
          {/* DDL Preview — the literal CREATE / ALTER statements the approval
              will execute against the target cluster. Pre-computed at submit
              time by the same code path the apply step uses, so what the
              architect sees here is exactly what will run. */}
          <div>
            <h4 className="text-sm font-semibold text-slate-700 mb-3 flex items-center gap-2">
              <svg className="w-4 h-4 text-slate-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            {ddlStatements.length === 0 ? (
              ...
              <div className="rounded-xl border border-slate-200 bg-slate-900 text-slate-100 overflow-hidden">
                <pre className="text-xs leading-relaxed font-mono whitespace-pre overflow-x-auto px-4 py-3">
                  {ddlStatements.map((stmt, i) => (
                    <div key={i} className="py-0.5">
                      <span className="text-slate-500 select-none mr-3">{String(i + 1).padStart(2, " ")}.</span>
                      <span>{stmt};</span>
                    </div>
                  ))}
                </pre>
